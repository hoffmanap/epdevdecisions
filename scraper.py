...
HEADERS = {
    ...
    'Referer': 'https://www2.elpasotexas.gov/municipal-clerk/City%20Plan%20Commission.html'
}
    ...
    if url.lower().endswith('.htm') or 'agenda.htm' in url.lower():
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"Warning: HTM resolver returned status {r.status_code} for {url}")
                return url
...
def extract_cpc_data(url, date):
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(response.content)) as pdf:
            ...
            # Split items based on Case Number formats (PZRZ, PZST, PL, SU, ZON, PWSU)
            ...
            for i in range(1, len(items), 2):
                ...
                # Extract Status (Keywords like 'Motion carried' or 'Approved')
...
def main():
    ...
    for index, row in links_df.iterrows():
        ...
        time.sleep(2) # Increased delay to be even safer with the server
    ...
    if all_results:
        ...
        def get_coords(addr):
            if addr == "Unknown" or len(addr) < 5: return None
            try:
                # Add 'El Paso, TX' for context
        ...
        print("No data extracted. Verify your links in agenda_links.csv.")
